...
object KafkaSourceFactory {
  ...
  def createKafkaConsumer(): FlinkKafkaConsumer[String] = {
    ...
    properties.setProperty("bootstrap.servers", "172.22.11.36:9092")
    properties.setProperty("group.id", "wufc-local")

    val kafkaConsumer = new FlinkKafkaConsumer[String]("ods_flow_sc_event_raw", new SimpleStringSchema(), properties)
